...
{
	while (!GET_SPI_TRANSMISSION_DONE);
    return SPI_DATA; // READING FROM THE AXI SPI REGISTER
}

static u32 spi_send_data(u32 data)
{
	while (!GET_SPI_TRANSMISSION_DONE);
	SPI_DATA = data; // WRITING ON THE AXI SPI REGISTER
    return spi_receive_data();
}
///////////////////////////////////////////////////////////////////////////////
static void MAX1163_initialize(void)
{
...
static u16 MAX1163_read(u8 channel)
{
    channel <<= 3;
    return (u16) (0xFFF & spi_send_data( ( CONVERSION | channel | NO_SCAN ) << 16 ));
}

static MAX1163 MAX1163_singleton = {MAX1163_initialize, MAX1163_read};

MAX1163 * MAX1163_instance(void)
{
	return & MAX1163_singleton;
}
///////////////////////////////////////////////////////////////////////////////
